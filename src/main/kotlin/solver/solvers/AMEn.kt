...
private enum class Direction { LR, RL }
...
fun AMEnSolve2(
    ...
    // Matlab uses column-first ordering of elements for the reshape operation,
    // while EJML's SimpleMatrix uses row-first
    fun matlabReshape(M: SimpleMatrix, rows: Int, cols: Int): SimpleMatrix {
        val res = M.transpose()
        M.reshape(cols, rows)
    ...
    for (swp in 0 until maxSweeps) {
        ...
        for (i in d-1 downTo 1) {
            if (swp > 0) {
                ...
                zAt = bfun3(phizA[i], A.tt.cores[i], phizA[i + 1], xCoreVect)
                val yCoreVect = y.tt.cores[i].leftUnfolding()
                yCoreVect.reshape(yCoreVect.numElements, 1)
                val zy = bfun2(phizy[i], phizy[i + 1], y.tt.cores[i])
                val znew = zy - zAt
        ...
        for(i in 0 until d) {
            ...
            val A1 = A.tt.cores[i]
            val y1 = y.tt.cores[i]
//            val solPrev = x.tt.cores[i].leftUnfolding()
//            solPrev.reshape(solPrev.numElements, 1)
            var rhs = bfun2(phiy[i], phiy[i+1], y1)
            ...
            if(i < d-1) {
                if (truncateBasedOnResidual) {
                    while (true) {
                        ...
                        var product = bfun3(phi1, A.tt.cores[i], phi2, currSol)
                        if(normalize)
            ...
            val crzy = bfun2(phizy[i], phizy[i+1], y1)
            val crzAt = bfun3(phizA[i], A1, phizA[i+1], truncSol)
            ...
            if(i < d-1) {
                ...
                // enrichment
                val leftresid = bfun3(phiA[i], A1, phizA[i+1], newU*modifier)
                val lefty = bfun2(phiy[i], phizy[i+1], y1)
...
fun computePsi(PsiPrev: TPhi, xCore: CoreTensor, ACore: CoreTensor?, yCore: CoreTensor): TPhi {
...
fun computePhi(PhiPrev: TPhi, xCore: CoreTensor, ACore: CoreTensor?, yCore: CoreTensor): TPhi {
    val res = arrayListOf<ArrayList<SimpleMatrix>>()
...
//TODO: name
fun bfun2(psi: TPhi, phi: TPhi, y: CoreTensor): SimpleMatrix {
...
//        for (beta1 in 0 until psi[0].size) {
            for (beta2 in 0 until phi.size) {
//            for (beta2 in 0 until phi[0].size) {
                var sum = 0.0
                for (gamma1 in 0 until psi[0].size) {
//                for (gamma1 in 0 until psi.size) {
                    for (gamma2 in 0 until phi[0].size) {
//                    for (gamma2 in 0 until phi.size) {
...
//                res[n * psi[0].size * phi[0].size + beta1 * phi[0].size + beta2] = sum
            }
        }
    }
    return res
